...
export function PlayerCard({ player }: { player: Player }) {
  const photoSrc = getValidImageSrc(player.photo);
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] bg-gradient-to-b from-brand-red/20 via-brand-black to-brand-black-2 overflow-hidden">
        ...
        <span className="absolute top-3 right-3 max-w-[45%] break-words text-right font-display text-4xl md:text-6xl font-bold text-brand-gold leading-none drop-shadow-[0_0_10px_rgba(0,0,0,0.8)]">
          {player.number}
        </span>
        <div className="absolute bottom-3 left-3 right-3">
          ...
          <p className="break-words font-display text-lg sm:text-xl uppercase leading-tight">{player.nickname}</p>
          <p className="break-words text-xs text-brand-gray">{player.name}</p>
        </div>
      </div>
